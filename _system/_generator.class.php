...
class ClassGenerator
{
    ...
    private function generateClasses($tables)
    {
        foreach ($tables as $table => $table_type) {
            if (!in_array($table, $this->skip_table)) {
                ...
                $content .= 'class ' . $class .  ' {' . NL . NL;//de aqui se crea la clase
                ...
                $content .= TAB . 'public static $TABLE_NAME = \'' . $table . '\';' . NL.NL;
                $and = '';
                $primary_key = '';
 ...
 // $content .= ');';
                ...
                foreach ($columns as $column) {
                    ...
                    $content .= TAB . 'public $' . $str_column . ' ;' . NL;
            ...
            $primary_key='';
                foreach ($columns as $key => $pKey) {
                    $str_column = str_replace($this->str_replace_column, '', $pKey);
                    $primary_key .= '$' . $pKey . ',';
                }//constructor
                $content .= TAB . 'function constructor(' . $primary_key . '){' ;
                ...
                foreach ($columns as $key => $pKey) {
                    ...
                    $primary_key .= TAB . '$this->' . $pKey . '=$'.$pKey.';'. NL;
                }//CONTENIDO CONSTRUCTOR
                $content .=  $primary_key.TAB . '}' . NL.NL;


                $primary_key='INSERT INTO(';
                foreach ($columns as $key => $pKey) {
                   
                    $primary_key .=  $pKey . ',';
                }//CONTENIDO CONSTRUCTOR

                 $content .='function $consulta="insertar(){'.NL;
                $content .=  $primary_key;

                $primary_key='}values(';
                foreach ($columns as $key => $pKey) {
                   
                    $primary_key .=  '".$this->'.$pKey . '.",';
                }//CONTENIDO CONSTRUCTOR
    ...
    private function verificarTipo($type,$texto)
    {
        if (strpos($type, 'int') !== FALSE) {
            return $texto;
